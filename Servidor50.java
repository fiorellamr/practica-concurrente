    ...
    void ServidorEnvia(String envia) {// El servidor tiene texto de envio
        if (envia != null) {
            ...
            if (envia.trim().contains("envio")) {
                ...
                String arrayString[] = envia.split("\\s+"); // SPLITEA ENVIA
                // System.out.println(arrayString[3]);
                System.out.println(envia);
                int data1 = Integer.parseInt(arrayString[2]);
                int data2 = Integer.parseInt(arrayString[3]);
                int data3 = Integer.parseInt(arrayString[4]);
                if (mTcpServer != null) {
                    mTcpServer.sendMessageTCPServerRango(envia, data1, data2, data3); // ENVIA EL RANGO PARTICIONADO POR
